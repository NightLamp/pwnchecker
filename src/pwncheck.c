 ...
 * 	$ cc -o pwncheck pwncheck.c -lssl -lcrypto
 * 
 */
...
int main(int argc, char ** argv) {
	...
	if ( (filePath = getenv("HOME")) == NULL) {
		...
		strcat(filePath, "/.pwncheck");
		
		if (filePath == NULL) {
			fprintf(stderr, "pwncheck: error finding homepath.\n");
		}
	}
		...
		if (strcmp(argv[1], "store") == 0) {
			...
			for (int a = 0; a < clSize; a++) {
				...
				if (fork() == 0) 	{
					...
					fprintf(stderr, "pwncheck: error calling curl\n");
					exit(EXIT_FAILURE);
				}			
				...
				if ( (fp = fdopen(pipfd[0], "r" )) == NULL) {
					fprintf(stderr, "pwncheck: fdopen error\n");
					exit(EXIT_FAILURE);
				}
				close(pipfd[1]);
